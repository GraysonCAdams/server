...
from .constants import (
    ...
    BASE_URL,
    CACHE_CATEGORY_OPML,
    CACHE_CATEGORY_PODCAST_ITEMS,
    CACHE_KEY_LAST_APPLIED,
    CONF_MAX_NUM_EPISODES,
    CONF_SESSION_COOKIE,
    FEED_CACHE_EXPIRATION,
    LOGIN_URL,
...
class OvercastProvider(MusicProvider):
    ...
    async def handle_async_init(self) -> None:
        ...
        if not self.get_setup_value(CONF_USERNAME) or not self.get_setup_value(CONF_PASSWORD):
            raise SetupFailedError("Missing Overcast email or password")
            ...
            async with self.http_session.get(OPML_EXPORT_URL, allow_redirects=False) as response:
                ...
                if response.status == 429:
                    raise ResourceTemporarilyUnavailable(
                        "Overcast OPML export is rate limited",
    ...
    async def _get_opml_subscriptions(self) -> dict[str, OvercastSubscription]:
        return parse_extended_opml(await self._fetch_opml_text())
    ...
    async def _get_episode_stream_url(self, podcast_id: str, guid_or_stream_url: str) -> str | None:
        ...
        for episode in podcast.get("episodes", []):
            ...
            episode_key = guid if guid is not None and len(guid.split(" ")) == 1 else stream_url
            if guid_or_stream_url == episode_key:
                return stream_url
        return None
    ...
    async def _cache_set_podcast(self, feed_url: str, parsed_podcast: dict[str, Any]) -> None:
        await self.mass.cache.set(
            ...
            category=CACHE_CATEGORY_PODCAST_ITEMS,
            data=parsed_podcast,
            expiration=FEED_CACHE_EXPIRATION,
        )
